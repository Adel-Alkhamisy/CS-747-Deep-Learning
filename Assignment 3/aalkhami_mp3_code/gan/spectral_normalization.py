...
class SpectralNorm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        for _ in range(self.power_iterations):
            ...
            v.data = l2normalize(torch.mv(torch.t(w.view(w.shape[0], -1).data), u.data))
            # Power iteration for u
            u.data = l2normalize(torch.mv(w.view(w.shape[0], -1).data, v.data))

        # Calculate w with the spectral norm
        sigma = u.data.dot(w.view(w.shape[0], -1).mv(v.data))
        w_sn = w / sigma

        # Update w in the module
