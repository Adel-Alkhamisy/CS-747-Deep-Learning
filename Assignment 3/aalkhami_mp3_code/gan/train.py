...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            d_fake = D(fake_images.detach())  # Detach fake images from the generator to avoid backpropagation through G
...
def train_GANGP(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
              batch_size=128, noise_size=100, num_epochs=10, train_loader=None, device=None,
              gradient_penalty_weight=10):  # Add gradient_penalty_weight parameter
    """
    Train loop for GAN-GP.
    ...
    - train_loader: image dataloader
    - device: PyTorch device
    - gradient_penalty_weight: Add gradient_penalty_weight parameter
    """
    ...
    for epoch in range(num_epochs):
        print('EPOCH: ', (epoch+1))
        for x, _ in train_loader:
            _, input_channels, img_size, _ = x.size()
            real_images = preprocess_img(x).to(device)  # normalize
            ...
            gp = gradient_penalty(D, real_images, fake_images.detach(), device)
            d_error = discriminator_loss(d_real, d_fake) + gradient_penalty_weight * gp
            d_error.backward()
            D_solver.step()
